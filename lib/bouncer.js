...
function createBouncer (type, serverConfig, fofConfig) {
  ...
  function requestHandler (req, res, bounce) {
    ...
    if (!hostname) {
      debug.error('Hostname is unknown, this may the sign of an attack!')
    }
    debug('%o requested %o %o', req.connection.remoteAddress, req.method, (req.connection.encrypted ? 'https' : 'http') + '://' + hostname + req.url)
